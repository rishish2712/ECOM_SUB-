...
import { approveRazorpayOrder, createRazorpayOrder } from '@/lib/actions/order.actions'
  ...
  const handleRazorpayPayment = async () => {
    ...
    const options = {
      ...
      handler: async (response: any) => {
        const verifyRes = await approveRazorpayOrder(order._id, response)
        toast({
          description: verifyRes.message,
          variant: verifyRes.success ? 'default' : 'destructive',
        })
        ...
        <div className='md:col-span-3'>
          ...
          <div className='grid md:grid-cols-3 my-3 pb-3'>
            ...
            <div className='col-span-2'>
              ...
              <ul>
                {items.map((item) => (
                  <li key={item.slug}>{item.name} x {item.quantity} = {item.price}</li>
                ))}
              </ul>
            </div>
          </div>
